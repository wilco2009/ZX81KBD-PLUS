...
// Leer dato de canal 'channel'
  ...
  if (Wire.available())
  {
    ...
    S[8]=0;
    if (JUP) bitClear(rows[2],0);   // else bitSet(rows[2],0); 
    if (JDOWN) bitClear(rows[1],0); // else bitSet(rows[1],0); 
    if (JLEFT) bitClear(rows[5],1); // else bitSet(rows[5],1); 
    if (JRIGHT) bitClear(rows[5],0); // else bitSet(rows[5],0); 
    //if (JBUTB) bitClear(rows[4],0); // else bitSet(rows[5],0); 
    if (JTRIGER) bitClear(rows[4],0); // else bitSet(rows[5],0); 
    if (JBUTA) bitClear(rows[6],0); // else bitSet(rows[5],0); 
  }
