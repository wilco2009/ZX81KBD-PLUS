...
void KbdRptParser::OnKeyDown(uint8_t mod, uint8_t sc)
{
  ...
  if (shiftPressed){
    ...
    if (sc==0x52) upPressed = true;
    if (sc==0x51) downPressed = true;
    if (sc==0x50) leftPressed = true;
    if (sc==0x4f) rightPressed = true;
...
void KbdRptParser::OnKeyUp(uint8_t mod, uint8_t sc)
{
  ...
  if (shiftPressed){
    ...
    if (sc==0x52) upPressed = false;
    if (sc==0x51) downPressed = false;
    if (sc==0x50) leftPressed = false;
    if (sc==0x4f) rightPressed = false;
    v = findUSBIntlCode(sc);
    if (v == (NULL  + 40)){
      v = pgm_read_byte_near(USBCodes+sc);
    };// else Serial.println(F("INTL"));
